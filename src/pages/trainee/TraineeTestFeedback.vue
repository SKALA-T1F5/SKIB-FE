...
const calculateRank = () => {
  ...
  for (let i = 0; i < sortedScores.length; i++) {
    ...
    // 현재 사용자의 점수와 일치하는 첫 번째 항목을 찾고 랭크를 설정 (동점자 처리 포함)
    // myUserId를 통해 정확히 "나"의 점수를 찾도록 변경
    if (
      allParticipantScores.value.some(
        (p) => p.userId === myUserId && p.score === sortedScores[i],
      ) &&
      !rankSet
    ) {
      // 여러 동점자 중 내 userId에 해당하는 첫 번째 등수를 찾음 (오름차순으로 등수를 매길 경우)
      // 또는 단순히 'myScore'를 기준으로 높은 점수 가진 사람 수 + 1
      const myParticipantScore = allParticipantScores.value.find((p) => p.userId === myUserId)
      if (myParticipantScore && myParticipantScore.score === sortedScores[i]) {
  ...
  // 만약 나의 점수가 allParticipantScores에 포함되어 있지 않거나 랭크가 설정되지 않은 경우 처리
  if (!rankSet && myScore !== undefined) {
    const higherScoresCount = sortedScores.filter((score) => score > myScore).length
    myRank.value = higherScoresCount + 1 // 나보다 점수 높은 사람 수 + 1이 나의 등수
  }
}
...
<style scoped>
/* 기존의 #feedback-page-layout 스타일은 MainLayout이 처리하므로 제거합니다. */
...
/* MainLayout의 .main-content가 padding을 가집니다. */
/* feedback-main-content는 최대 너비 및 중앙 정렬만 담당하도록 조정합니다. */
.feedback-main-content {
  flex-grow: 1;
  /* padding: 30px; 이 부분은 MainLayout의 .main-content에서 이미 처리되므로 제거하거나 조정 */
